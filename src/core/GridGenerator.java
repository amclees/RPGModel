...
public class GridGenerator {
  public static Grid makeGrid() {
    ...
    while (bandits > 0) {
      ...
      if (Math.random() < 0.5)
        ((ICharacter) grid.getElement(i, j, Layer.CHARACTER)).equip(EquipmentRegistry.getEquipment("Bow"),
    ...
    while (pirates > 0) {
      ...
      ((ICharacter) grid.getElement(i, j, Layer.CHARACTER)).equip(EquipmentRegistry.getEquipment("Cutlass"),
          EquipmentSlot.RIGHTHAND);
    }
    return grid;
  }
