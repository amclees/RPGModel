...
public class Grid {
    ...
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        if (!(i == 0 && j == 0)) {
          ...
          try {
            if (this.getElement(neighbor[0], neighbor[1], layer) != null)
              neighbors.add(this.getElement(neighbor[0], neighbor[1], layer));
  ...
  public boolean hasCharacter(int x, int y) {
    if (this.getElement(x, y, Layer.CHARACTER) == null)
      return false;
    else
      return true;
  ...
  public boolean isClear(int x, int y) {
    ...
    if (terrain != null)
      clear = clear && terrain.isPassable();
